...
function buildWsUrl(): string {
  ...
  const host = import.meta.env.DEV
    ...
    : location.host;
  return `${wsProtocol}//${host}/api/hermes/terminal${
    token ? `?token=${encodeURIComponent(token)}` : ""
  }`;
}
...
function connect() {
  ...
  ws.onclose = () => {
    if (sessions.value.length === 0 && !wsErrorNotified) {
      wsErrorNotified = true;
      message.error(t("terminal.wsConnectFailed"));
    }
